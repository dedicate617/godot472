			...
			const unsigned short &namespaceId)
{
	...
	std::vector<UA_Argument> uaInputArguments;
	//UA_Argument uaInputArguments[inputArguments.size()];
	size_t i=0;
	for(auto input = inputArguments.begin(); input != inputArguments.end(); input++) {
		UA_Argument uaInputArgument;
		UA_Argument_init(&uaInputArgument);
		uaInputArgument.description = UA_LOCALIZEDTEXT_ALLOC("en-US", input->first.c_str());
		uaInputArgument.name = UA_STRING_ALLOC(input->first.c_str());
		// get a value copy
		std::shared_ptr<Variant::NativeVariantType> valuePtr = input->second.getInternalValuePtr();
		uaInputArgument.dataType = valuePtr->type->typeId;

		if(input->second.isScalar()) {
			// scalar type
			uaInputArgument.valueRank = UA_VALUERANK_SCALAR;
		} else {
			// setup the array dimensions
			uaInputArgument.valueRank = UA_VALUERANK_ONE_DIMENSION;
			uaInputArgument.arrayDimensionsSize = valuePtr->arrayDimensionsSize;
			uaInputArgument.arrayDimensions = UA_UInt32_new();
			UA_UInt32_copy(valuePtr->arrayDimensions, uaInputArgument.arrayDimensions);
		}
		uaInputArguments.push_back(uaInputArgument);
	...
	std::vector<UA_Argument> uaOutputArguments;
	...
	{
		UA_Argument uaOutputArgument;
		UA_Argument_init(&uaOutputArgument);
		uaOutputArgument.description = UA_LOCALIZEDTEXT_ALLOC("en-US", output->first.c_str());
		uaOutputArgument.name = UA_STRING_ALLOC(output->first.c_str());
		// get a value copy
		std::shared_ptr<Variant::NativeVariantType> valuePtr = output->second.getInternalValuePtr();
		uaOutputArgument.dataType = valuePtr->type->typeId;

		if(output->second.isScalar()) {
			// scalar type
			uaOutputArgument.valueRank = UA_VALUERANK_SCALAR;
		} else {
			// setup the array dimensions
			uaOutputArgument.valueRank = UA_VALUERANK_ONE_DIMENSION;
			uaOutputArgument.arrayDimensionsSize = valuePtr->arrayDimensionsSize;
			uaOutputArgument.arrayDimensions = UA_UInt32_new();
			UA_UInt32_copy(valuePtr->arrayDimensions, uaOutputArgument.arrayDimensions);
		}
		uaOutputArguments.push_back(uaOutputArgument);
	...
	UA_StatusCode status = UA_Server_addMethodNode(server,
			...
			*rootObjectIdPtr, // parent is the specified root object
			...
			inputArguments.size(), uaInputArguments.data(),  // set input arguments
			outputArguments.size(), uaOutputArguments.data(), // set output arguments
			NULL, NULL);
