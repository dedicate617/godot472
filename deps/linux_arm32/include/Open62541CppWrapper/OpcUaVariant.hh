...
static std::unordered_map<std::type_index, UA_UInt16> UA_TypeMappings =
{
		...
		{ std::type_index(typeid(UA_String)), UA_TYPES_STRING },
		{ std::type_index(typeid(UA_DateTime)), UA_TYPES_DATETIME }
};
...
public:
	...
	std::string UTF8ToString(std::string& utf8Data) const;
...
inline std::vector<std::string> Variant::getArrayValuesAs() const
{
	...
	// case 1: for open62541 v1.1
	//if(value->type->typeIndex == UA_TYPES_STRING) {
	// case 2: for open62541 v1.3.5
...
{
	UA_Variant_clear(this->value.get());
