...
public:
	...
	HRESULT STDMETHODCALLTYPE Invoke(HRESULT p_error_code) {
		...
		imgdata.resize(size);
		Vector<uint8_t>::Write wr = imgdata.write();
...
public:
	...
	WebViewOverlayDelegate(WebViewOverlay* p_control, HWND p_hwnd) {
		...
		HRESULT hr = webview_CreateCoreWebView2EnvironmentWithOptions(nullptr, (LPCWSTR)cache_path.to_wchar_buffer().ptrw(), nullptr, this);
		...
		case NOTIFICATION_INTERNAL_PROCESS: {
			...
			if (!Engine::get_singleton()->is_editor_hint() && (data->view != nullptr) && (data->view->is_ready) && (ctrl_err_status == -1) && (err_status == 0)) {
				...
				if (no_background) {
					...
					HRESULT hr = data->view->controller->QueryInterface(IID_ICoreWebView2Controller2, (void**)&controller2);
					ERR_FAIL_COND_V(FAILED(hr));
					if (controller2) {
				...
				if (!context_menus_enable) {
					ComPtr<ICoreWebView2Settings> settings;
					data->view->webview->get_Settings(&settings);
					// 禁止dev菜单
				...
				data->view->webview->Navigate((LPCWSTR)home_url.to_wchar_buffer().ptrw());
				ctrl_err_status = 0;
...
void WebViewOverlay::set_url(const String& p_url) {
	home_url = p_url;
	if ((data->view != nullptr) && (data->view->is_ready)) {
		data->view->webview->Navigate((LPCWSTR)p_url.to_wchar_buffer().ptrw()); // utf8().get_data());
	}
}
...
void WebViewOverlay::execute_java_script(const String &p_script) {
	...
	data->view->webview->ExecuteScript((LPCWSTR)p_script.to_wchar_buffer().ptrw(), nullptr);
}

void WebViewOverlay::load_string(const String &p_source) {
	ERR_FAIL_COND(data->view == nullptr || !data->view->is_ready);
	data->view->webview->NavigateToString((LPCWSTR)p_source.to_wchar_buffer().ptrw());
}
...
void WebViewOverlay::init() {
	HMODULE wv2_lib = LoadLibraryW(L"plugins/WebView2Loader.dll");
	if (wv2_lib) {
