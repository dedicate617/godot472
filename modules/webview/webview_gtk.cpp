...
//typedef void * (*gtk_socket_new_ptr)(void);
//typedef void * (*gtk_socket_get_id_ptr)(void *socket);
...
//gtk_socket_new_ptr webview_gtk_socket_new = nullptr;
//gtk_socket_get_id_ptr webview_gtk_socket_get_id = nullptr;
		...
		case NOTIFICATION_INTERNAL_PROCESS: {
			if (!Engine::get_singleton()->is_editor_hint() && (data->view == nullptr) && (err_status == 0)) {
				...
				if (data->container == nullptr) {
					...
					void *xwindow = (void *)DisplayServer::get_singleton()->window_get_native_handle(DisplayServer::WINDOW_HANDLE);
					//void *xdisplay = OS::get_singleton()->get_native_handle(OS::DISPLAY_HANDLE);
					void *xdisplay = (void *)DisplayServer::get_singleton()->window_get_native_handle(DisplayServer::DISPLAY_HANDLE);
					...
					GdkDisplay *display = webview_gdk_x11_lookup_xdisplay(xdisplay);
					g_message("GdkDisplay=%d", display);
					GdkWindow *window = webview_gdk_x11_window_foreign_new_for_display(display, xwindow);
					g_message("GdkWindow=%d", window);
					data->container = gtk_fixed_new();  // linux系统中设置godot为parent时gtk_fixed_new会导致崩溃
				...
				//	g_message("GdkWindow=%d", window);
				//	//XMapRaised(webview_gdk_x11_lookup_xdisplay(xdisplay), window);
				//	//data->container = gtk_fixed_new();
				//	data->container = gtk_window_new(GTK_WINDOW_TOPLEVEL);
				//	//data->container = gtk_widget_new(GTK_TYPE_WINDOW, NULL);
				//	//g_signal_connect(data->container, "realize", G_CALLBACK(data->my_gtk_realize), window);
				//	//gtk_widget_set_has_window(data->container, TRUE);
				//	//gtk_widget_realize(data->container);

				//	gtk_widget_set_window(GTK_WIDGET(data->container), window);
				//	g_message("gtk_widget_set_window");
				//	gtk_widget_set_visible(GTK_WIDGET(data->container), TRUE);
				//	g_message("gtk_widget_set_visible");
				//}

				//GtkWidget *webview = (GtkWidget*)gtk_socket_new();  // webview as socket
				//g_message("webview=%d,container=%d", webview, data->container);
				//gtk_widget_show(webview);
				//gtk_container_add(GTK_CONTAINER(data->container), webview);
				//gtk_widget_show(data->container);
				//g_message("socket_id=%d", gtk_socket_get_id((GtkSocket*)webview));
...
void WebViewOverlay::set_context_menus_enable(bool p_context_menus_enable) {
	context_menus_enable = p_context_menus_enable;
}

bool WebViewOverlay::get_context_menus_enable() const {
	return context_menus_enable;
}
...
void WebViewOverlay::init() {
	...
	//void *gdk3_lib = dlopen("libgdk-x11-2.0.so", RTLD_LAZY);
	if (gdk3_lib == nullptr) {
		err_status = 1;
		return;
	}
	webview_gdk_x11_lookup_xdisplay = (gdk_x11_lookup_xdisplay_ptr)dlsym(gdk3_lib, "gdk_x11_lookup_xdisplay");
	g_message("webview_gdk_x11_lookup_xdisplay=%d", webview_gdk_x11_lookup_xdisplay);
	...
	g_message("webview_gdk_x11_window_foreign_new_for_display=%d", webview_gdk_x11_window_foreign_new_for_display);
	...
	//void *gtk3_lib = dlopen("libgtk-x11-2.0.so", RTLD_LAZY);
	//webview_gtk_socket_new = (gtk_socket_new_ptr)dlsym(gdk3_lib, "gtk_socket_new");
	//g_message("webview_gtk_socket_new=%d", webview_gtk_socket_new);
	//webview_gtk_socket_get_id = (gtk_socket_get_id_ptr)dlsym(gdk3_lib, "gtk_socket_get_id");
	//g_message("webview_gtk_socket_get_id=%d", webview_gtk_socket_get_id);
	if (gtk3_lib == nullptr/* || webview_gtk_socket_new == nullptr || webview_gtk_socket_get_id == nullptr*/) {
		err_status = 3;
		return;
	}
	/*
